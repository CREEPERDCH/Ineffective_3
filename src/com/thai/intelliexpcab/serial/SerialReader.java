...
public class SerialReader extends Observable implements Runnable, SerialPortEventListener {
    ...
    public void open(HashMap params) {
        ...
        try {
            ...
            serialPort = (SerialPort) portId.open("SerialReader", timeout);//使用者  和 最大响应时长(ms)
            ...
            isOpen = true;
        } catch (PortInUseException | TooManyListenersException | NoSuchPortException | UnsupportedCommOperationException | IOException e) {
...
//                outputStream.write(message.getBytes()); //往串口发送数据，是双向通讯的。
                outputStream.write(SerialTool.hex2byte(message)); //往串口发送数据，是双向通讯的。
            }
        } catch (IOException e) {
        }
    }
    ...
    public void serialEvent(SerialPortEvent event) {
        ...
        switch (event.getEventType()) {
            ...
            case SerialPortEvent.RI: // 5 响铃侦测
            case SerialPortEvent.OUTPUT_BUFFER_EMPTY: // 2 输出缓冲区已清空
                break;
            case SerialPortEvent.DATA_AVAILABLE: // 1 有数据到达
                try {
                    // 多次读取,将所有数据读入
